...
- **C99的指定初始化器**（Designated Initializer）：C99标准允许你初始化特定的元素，这对于**大型稀疏数组（大部分元素为0）**非常方便。例如：`c下载复制运行int arr[3][4] = { [0][0] = 1, [1][1] = 2, [2][2] = 3 };`这会将数组左上角到右下对角线的三个位置初始化为1、2、3，其余元素自动为0。
