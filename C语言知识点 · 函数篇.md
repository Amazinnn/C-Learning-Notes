...
> 参数传递过程中，既可以将实参的值传递给形参，也可以将形参的值传递给实参，这种参数传递是双向的。 
...
- **主调函数外部声明**：这种声明属于**全局声明**。==通常，我们会将函数声明放在源文件的开头（可以不在开头）==，`#include`指令的下面。这样，声明之后的所有函数就都能调用它了 。这是更常见和推荐的做法，因为它避免了重复声明，让代码更清晰。
...
### 17、在一段程序中不调用函数而直接将两个整型变量a和b的值互换，一定可以不借助第三个变量完成的吗？
...
在C语言中，函数名的行为**非常像指针**，因为它代表函数的入口地址，但严格来说，它本身并不是一个指针变量。

**函数名的本质**

- **地址的代号**：函数名是编译器在编译阶段用来代表函数入口地址的符号。程序加载到内存后，函数入口地址固定，编译器会将代码中的函数名替换为这个固定地址。
- **地址常量**：函数名是一个**地址常量**，其值（即函数地址）在程序生命周期内不可改变。你不能像对指针变量那样给函数名赋值，让它指向其他函数。

**函数名与函数指针的区别**

​	尽管函数名可以自动转换为函数地址，但它与函数指针变量有本质区别：

| 特性         | 函数名                                       | 函数指针变量                          |
| ------------ | -------------------------------------------- | ------------------------------------- |
| **本质**     | 地址常量，是函数的符号代号                   | 真正的变量，存储一个地址值            |
| **内存占用** | 本身不占用额外内存                           | 占用内存（通常4或8字节）来存储地址    |
| **可变性**   | 值（代表的地址）不可变                       | 值可以改变，可以指向不同函数          |
| **取址操作** | `&函数名`和 `函数名`含义相同，都得到函数地址 | `&函数指针`得到的是指针变量自身的地址 |

​	例如，对于函数 `void func(int)`，`func`和 `&func`的值是相同的，都指向函数的起始地址。而声明一个函数指针 `void (*pfunc)(int) = func;`后，`pfunc`是一个变量，存储着 `func`的地址，`&pfunc`则是变量 `pfunc`自身的地址。

​	由于**函数名能自动转换为地址**，所以它可以：

- **直接赋值给函数指针**：`pfunc = func;`（等价于 `pfunc = &func;`）。
- **直接调用函数**：`func(5);`和 `(*pfunc)(5);`或 `pfunc(5);`效果相同。后两种方式通过指针调用，本质上是先取得指针存储的地址，再跳转执行。

​	简单来说，**函数名是一个代表固定地址的符号，而函数指针是一个存储地址的变量**。理解这一点对深入掌握C语言函数机制很有帮助。
...
int main() {
    int a = 10, b = 5;
    int choice;
    printf("请输入操作类型 (1:加法, 2:减法, 3:乘法): ");
    scanf("%d", &choice);
    // 4. 声明函数指针变量
    Operation current_operation = NULL;
    // 5. 根据输入动态选择函数（体现灵活性）
    switch(choice) {
        case 1: current_operation = add; break;
        case 2: current_operation = subtract; break;
        case 3: current_operation = multiply; break;
        default: 
            printf("无效输入！\n");
            return 1;
    }
    ...
    if(current_operation != NULL) {
        int result = calculate(a, b, current_operation);
        printf("结果: %d\n", result);
    }
    return 0;
}
...
// 2. 注意参数类型：直接声明函数指针类型
int calculate(int x, int y, int (*op)(int, int)) { // 关键区别在这里
    return op(x, y);
}
...
int main() {
    ...
    scanf("%d", &choice);
    // 3. 直接声明函数指针变量
    int (*current_operation)(int, int) = NULL; // 关键区别在这里
    switch(choice) {
        case 1: current_operation = add; break;     // 赋值不变，函数名即是地址
        case 2: current_operation = subtract; break;
        case 3: current_operation = multiply; break;
        default: 
            printf("无效输入！\n");
    ...
    // 4. 调用方式完全不变
    if(current_operation != NULL) {
        int result = calculate(a, b, current_operation); // 传递函数指针
        printf("结果: %d\n", result);
    }
    return 0;
}
...
通过该指针可以调用它所指向的函数。定义时必须明确返回类型和参数列表，例如 `int (*func_ptr)(int, int);`。

**typedef 简化**

使用 `typedef`为复杂的函数指针类型创建别名。

这能显著提高代码可读性和可维护性，例如 `typedef int (*Operation)(int, int);`，之后便可使用 `Operation op;`来声明变量。

**赋值与调用**

赋值时，函数名本身就代表其地址，`&`运算符可省略。调用时，有两种完全等效的语法。

\- **赋值**：`op = add;`（等价于 `op = &add;`）
\- **调用**：`result = op(3, 5);`或 `result = (*op)(3, 5);`

**核心应用：回调函数**

将函数指针作为参数传递给另一个函数，实现**动态行为绑定**。

在计算器程序中，`calculate`函数并不关心具体执行何种运算，它只负责调用传入的函数指针。具体的计算逻辑（如 `add`, `subtract`）由调用者决定，这极大地增强了代码的灵活性。

**高级数据结构**

可以创建**函数指针数组**，或将函数指针作为**结构体成员**。

\- **函数指针数组**：适用于实现命令模式或状态机，例如 `int (*ops[])(int, int) = {add, subtract};`。
\- **结构体成员**：可以模拟面向对象编程中的“方法”，将相关操作封装在一起。