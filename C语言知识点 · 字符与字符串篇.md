...
| :----------------------- | :------------------------------------ | :-------------------- | :------------------------------------------------ |
| **使用字符串常量初始化** | `char str[] = "Hello";`               | **可以**              | 这是C语言标准允许的==语法糖==，编译器会自动处理。 |
...
### 8.  当输入缓冲区包含前导空格（如"_ _ _ _1_22_333"）时，多次调用`scanf`会如何工作？ ###
...
### 13、scanf函数会将加减号等符号也扫描进去吗？

​	这个问题很好！`scanf("%d")`在处理加减号时，行为会根据符号出现的位置而有所不同。简单来说，它**能理解作为数字一部分的正负号，但会把单独或出现在错误位置的运算符当作输入错误**。

​	为了让你快速把握要点，我先用一个表格来总结 `%d`遇到不同符号时的反应：

| 符号位置             | 符号     | `scanf("%d", &num)`的行为            | 结果示例（输入 → `num`的值）       |
| -------------------- | -------- | ------------------------------------ | ---------------------------------- |
| **数字前**           | `+`/ `-` | **正常读取**，将其视为正负号         | `+10`→ `10`/ `-10`→ `-10`          |
| **数字后（紧跟）**   | `+`/ `-` | **停止读取**，符号留在输入缓冲区     | `10+`→ `10`( `+`留在缓冲区)        |
| **数字后（有空格）** | `+`/ `-` | **停止读取**，空格和符号都留在缓冲区 | `10 +`→ `10`(空格和 `+`留在缓冲区) |
| **单独出现（开头）** | `+`/ `-` | **读取失败**，符号留在输入缓冲区     | `+`→ 读取失败 ( `+`留在缓冲区)     |

#### 💡 核心机制与注意事项 ####

​	理解表格中的行为，关键在于明白 `%d`的工作方式：它会**跳过空白字符**（如空格、换行），然后期望找到一个**可选的正负号（`+`或 `-`）后面紧跟数字序列**。当它遇到第一个不属于数字的字符时，就认为这个整数的输入结束了，并将这个字符**放回输入缓冲区**，等待下一次读取 。

#### ⚔️ `%d`与 `%c`读取符号的差异 ####

​	需要注意的是，`%d`和 `%c`对符号的处理方式完全不同。`%d`将 `+`/`-`视为数字的一部分，而 **`%c`会读取任何一个字符，包括空格和换行符** 。如果你希望 `%c`也跳过空白字符，可以在格式字符串中的 `%c`前加一个空格，如 `scanf(" %c", &ch)`。



### 在如下情况，会得到\与后面的字符，还是会得到转义字符？

> 在交互界面（譬如exe）输入\0或\n或\t等转义字符，然后使用scanf、gets、getchar等函数。

​	在C语言中，当你在交互界面输入`\0`、`\n`、`\t`这样的字符序列时，输入函数通常会将其视为两个独立的字符：反斜杠`\`和紧随其后的`0`、`n`、`t`等，而**不会**将其自动转换为转义字符所代表的含义（如空字符、换行符、制表符）。这是因为转义字符的处理发生在编译阶段，而输入函数在运行时接收的是你直接键入的原始字符序列。

💡 **理解背后的原理**

1. **编译时与运行时的区别**：转义字符（如`\n`）是C语言源代码中的特殊标记。当编译器遇到它们时，会将其转换为相应的单个控制字符（如换行符的ASCII码10）。而你在程序运行时，通过键盘输入的`\n`只是一个由反斜杠和字母`n`组成的普通字符序列，输入函数没有责任对其进行转义解析。
2. **输入缓冲区的作用**：你按下的每个键（包括反斜杠`\`、字母`n`以及最后的回车键）都会依次进入输入缓冲区。输入函数从缓冲区中按顺序读取这些字符，并不会将它们“拼接”成有特殊意义的转义字符。
...
**B.	const char *p=“I am a student”;**

**C.	char str[20];  str=“I am a student.”;**

**D.	strcpy ( str,”I am a student.”) ;**

| 选项                                          | 语法说明                   | 正确/错误  | 关键原因分析                                                 |
| --------------------------------------------- | -------------------------- | ---------- | ------------------------------------------------------------ |
| **A. `char string[]="I am a student.";`**     | 字符数组的**定义时初始化** | ✅ 正确     | 编译器会自动计算数组大小并在末尾添加空字符 `\0`，这是标准且推荐的初始化方式。 |
| **B. `const char \*p="I am a student";`**     | 字符指针指向字符串常量     | ✅ 正确     | 指针变量 `p`可以存储字符串常量的地址。使用 `const`修饰是良好实践，表明不应通过指针修改字符串内容。 |
| **C. `char str[20]; str="I am a student.";`** | 对已定义的数组名直接赋值   | ❌ **错误** | 数组名 `str`是一个**地址常量**，代表数组首元素的固定地址，不能被重新赋值。它不能作为左值出现在赋值运算符的左边。 |
| **D. `strcpy(str, "I am a student.");`**      | 使用标准库函数复制字符串   | ✅ 正确     | `strcpy`函数的功能就是将源字符串（包括结尾的 `\0`）复制到目标字符数组中。这是为已声明的数组赋值的标准方法。 |
...
####1、假设`scanf`语句执行时输入`ABCDE`<回车>，能使`puts(s)`语句正确输出`ABCDE`字符串的程序段是__________________________________________。

（我感觉四个选项都有点问题？A选项与B选项没有在字符串的末尾添加'\0'，C选项与D选项的s都是野指针。）
...
根据题目中的定义 `char str[] = "abc\000def\n"`，其中 `\000`是==八进制转义序列表示的空字符（数字零）==，需要求解字符串长度 `len`和数组大小 `size`。

1. **字符串长度 **：在C语言中，字符串的长度由第一个空字符 `'\0'`之前的字符个数决定。字符串字面量 `"abc\000def\n"`中的 `\000`表示一个空字符，因此字符串在字符 `'c'`后即终止。有效字符为 `'a'`、`'b'`、`'c'`，所以 `len = 3`。
2. **数组大小 **：当使用字符串字面量初始化字符数组时，数组的大小包括字面量中所有显式字符以及编译器自动添加的结尾空字符。字面量 `"abc\000def\n"`中的字符依次为： `'a'`、`'b'`、`'c'`：3个字符 `\000`：1个空字符 `'d'`、`'e'`、`'f'`：3个字符 `\n`：1个换行符 显式字符总数为8个，加上编译器隐式添加的结尾空字符，数组共有9个字符，即 `size = 9`。
